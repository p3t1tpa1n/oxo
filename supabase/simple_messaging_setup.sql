-- Script simplifié pour créer les tables de messagerie
...
-- 2. Créer la table conversation_participants
...
-- 5. Créer les politiques de sécurité pour conversations
...
CREATE POLICY "Utilisateurs peuvent créer des conversations" ON public.conversations
    FOR INSERT WITH CHECK (true);

-- 6. Créer les politiques pour conversation_participants
...
CREATE POLICY "Utilisateurs peuvent voir les participants de leurs conversations" ON public.conversation_participants
    FOR SELECT USING (
        conversation_id IN (
            SELECT conversation_id 
            FROM public.conversation_participants 
...
CREATE POLICY "Utilisateurs peuvent ajouter des participants" ON public.conversation_participants
    FOR INSERT WITH CHECK (true);

-- 7. Créer les politiques pour messages
...
CREATE POLICY "Utilisateurs peuvent modifier leurs messages" ON public.messages
    FOR UPDATE USING (
        conversation_id IN (
            SELECT conversation_id 
            FROM public.conversation_participants 
...
-- 8. Créer la fonction create_conversation
...
-- 9. Créer la fonction get_user_conversations
